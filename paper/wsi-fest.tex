...
% We analyze sense vectors based on their distance, which is not directly
% determined by the distance of the sense vectors in the source space because
...
% computation we keep the whole integer matrix of forward NN ranks in memory). We
% found that a vocabulary cutoff of $2^{15}=32768$ both on the source and the
% target size yields better results than the more ambitious $2^{16}=65536$. This
% is not the case for forward NN seach, where accuracy increases with vocabulary
% limit, see \cref{tab:prec}.

\subsection{Orthogonal restriction and other tricks} 
%Linear algebraic secret soauces}%length normalization, and centering}
