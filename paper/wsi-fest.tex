...
\todo{orthogonality \cite{Xing:2015}, CCA \citep{Faruqui:2014}, and preserving
analogies betwwen the mapped points \citep{Artetxe:2016}}
...
least one good translation among its \todo{10} rNNs. \Cref{tab:prec} shows results by
the best models.
...
only in \todo{\%} of the words with more than one sense vector.
  \todo{recompute the whole tabular with recall values}
...
arbitrary.}).  Relying on $s$ is similar to clustering the sense vectors for
